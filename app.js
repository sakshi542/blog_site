    ...
    Item.find({})
        .then(foundItems => {
            if (foundItems.length === 0) {
                ...
                item.save();
                res.render("home", { startingContent: item.title, posts: [item] });
            } else {
                res.render("home", { startingContent: foundItems[0].title, posts: foundItems });
...
app.get("/posts/:postName", function (req, res) {
    const requestedTitle = _.startCase(_.toLower(req.params.postName));

    Item.findOne({ title: requestedTitle })
        .then(foundItem => {
            if (foundItem) {
                res.render("post", {
